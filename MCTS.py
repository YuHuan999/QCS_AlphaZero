...
class MCTS(object):
    ...
    def playout(self, circuit):
        ...
        # list_actions_probs = list(actions_probs)
        # print("polic_value:",list_actions_probs, leaf_value )

        # #看电路是否满足合成要求：
        # is_target, product_HS= circuit.is_target()
        # reward = -1 + (1 / (4 ** CONFIG["n_qubit"])) * product_HS ** 2  #当前step的reward或者cost
...
class MCTSplayer(object):
    ...
    def __init__(self, policy_value_fn, c_puct=5, n_playout=2000, is_selfplay=1):
    ...
    def get_action(self, circuit, num_actions, temp=1e-3, return_prob=1):
