...
def array2num(array):  #根据数组找key
    return list(filter(lambda string: (num2array[string] == array).all(), num2array))[0]


# 压缩存储
def state_list2state_num_array(state_list):  #棋盘状态从key到array
...
def recovery_state_mcts_prob(tuple):
    ...
    state, move, mcts_prob, reward = tuple
    # state = recovery_array(state)
    # mcts_prob = recovery_array(mcts_prob)
    # print("type:state, mcts_prob", type(state), type(mcts_prob))
    # state = prepare_input(state)
    # mcts_prob = mcts_prob.reshape(12)
    # print("type:state, mcts_prob", type(state), type(mcts_prob))
    return state,mcts_prob,reward
